...
function runmain {
    ...
    python3 -m dg main.dg --summary --loop $N --pc-yes $pc --first-pref-ratio $fpr --nvb $halfpartic "$year/$state-gvts.csv" "$year/$state-first-prefs.csv" >> $file
...
for state in NSW VIC TAS WA SA QLD
  do for pc in 5 15 25 35 45 55 65 75 85 95
    do
    file="$year/results-sim-random-but-popular-$state-$pc.txt"
    echo $file
    rm $file &>/dev/null
